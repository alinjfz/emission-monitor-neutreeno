...
import type { SortDirection, SortName, StatusFilter, SubmissionList } from '@/types/api'
...
export function useSubmissionsQuery(query: SubmissionQuery, refreshKey: number) {
  ...
  const [state, setState] = useState<{ key: string; data: SubmissionList | null; error: string }>({
    key: '',
    data: null,
    error: '',
  })
  ...
  useEffect(() => {
    ...
    submissionsApi
      ...
      .catch((caught: unknown) => {
        ...
        const message = caught instanceof ApiError ? caught.message : 'Submissions could not be loaded.'
        setState((current) => ({ key: queryKey, data: current.data, error: message }))
      })
    return () => controller.abort()
  }, [query.status, query.search, query.sort, query.direction, query.page, query.pageSize, queryKey])
