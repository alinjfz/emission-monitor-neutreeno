      ...
      <div className="flex items-center justify-between">
        ...
        <CollapsibleTrigger asChild>
          <Button
            ...
            size="sm"
            aria-label={
              open ? "Collapse review history" : "Expand review history"
            }
          >
            {open ? "Hide" : "Show"}{" "}
            <ChevronDown
          ...
          events.map((event) => (
            <article key={event.id} className="rounded-lg border p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                {event.action === "opened" ? (
                  <span className="inline-flex items-center gap-1 text-xs font-medium text-[var(--info-text)]">
                    <MessageSquareText className="size-3" /> Opened
                  </span>
                ) : (
              ...
              {event.comment && (
                <p className="mt-2 text-sm leading-5">{event.comment}</p>
              )}
            </article>
          ))
        )}
