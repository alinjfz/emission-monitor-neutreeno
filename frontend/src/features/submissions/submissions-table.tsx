  ...
  return (
    ...
    >
      ...
      <span className="sr-only">
        {active ? `sorted ${direction}` : "not sorted"}
      </span>
    </Button>
  )
}
        ...
        <TableHeader>
          <TableRow className="bg-[#fafafa] hover:bg-[#fafafa]">
            ...
            {visibility.methodology && (
              <TableHead className="min-w-72">Methodology</TableHead>
            )}
            {visibility.latest_review && (
              <TableHead className="min-w-56">Latest review</TableHead>
            )}
          ...
          {submissions.map((submission) => (
            <TableRow key={submission.id} className="align-top">
              <TableCell>
                ...
                >
                  <span className="block font-medium text-foreground group-hover:underline group-hover:underline-offset-2">
              ...
              {visibility.footprint && (
                <TableCell>
                  <p className="font-medium tabular-nums">
                    {formatDecimal(submission.footprint_value)}
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {UNIT_LABELS[submission.unit_code]}
                  </p>
              ...
              {visibility.uncertainty && (
                <TableCell>
                  <span className="tabular-nums">
                    {formatDecimal(submission.uncertainty)}%
                  </span>
              ...
              {visibility.period && (
                <TableCell className="whitespace-nowrap text-muted-foreground">
                  {formatDate(submission.period_start)}
                  <br />
                  to {formatDate(submission.period_end)}
                </TableCell>
              )}
              {visibility.duration && (
              ...
              {visibility.submitted && (
                <TableCell className="whitespace-nowrap text-muted-foreground">
                  {formatDate(submission.submitted_at.slice(0, 10))}
                </TableCell>
              )}
              {visibility.last_modified && (
                <TableCell className="whitespace-nowrap text-muted-foreground">
                  {formatDateTime(submission.last_modified_at)}
                </TableCell>
              )}
              {visibility.methodology && (
                    ...
                    <div className="space-y-1">
                      <p>
                        <span className="font-medium text-foreground capitalize">
                          {submission.latest_review.action}
                        </span>{" "}
                        by {submission.latest_review.reviewer.name}
                      </p>
                      <p>
                        {formatDateTime(submission.latest_review.created_at)}
                      </p>
                      {submission.latest_review.comment && (
                        <p title={submission.latest_review.comment}>
                          “{truncate(submission.latest_review.comment)}”
                        </p>
                      )}
                    </div>
                  ) : (
